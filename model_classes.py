...
class ChooSiowPrimitives:
    ...
    def describe(self):
        X, Y = self.Phi.shape
        print_stars(
            "We are working with a Choo and Siow homoskedastic market"
        )
        print(
            f"\t\t we have {int(X)} types of men and {int(Y)} types of women"
        )
...
class NestedLogitPrimitives:
    ...
    def __init__(
        ...
        if -1 in nests_check or len(set(nests_check)) != len(
            nests_for_each_y
        ):

...
def estimate_semilinear_mde(
    ...
    Example:
        ...
        mde_results = estimate_semilinear_mde(
            mus_sim, phi_bases, entropy_model, additional_parameters=additional_parameters
        )
    ...
    if not parameterized_entropy:  # we only have e0(mu,r)
        ...
        if hessian == "provided":
            if additional_parameters is None:
                ...
                e0_derivative_mumu = cast(
                    EntropyHessianMuMu, e0_derivative[0]
                )
                ...
                e_derivative1 = cast(
                    EntropyHessiansParam, entropy.e_derivative
                )

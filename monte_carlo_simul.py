...
def choo_siow_simul(
    ...
    for s in range(n_simuls):
        seed_s = cast(int, child_seeds[s])
        mus_sim = choo_siow_instance.simulate(n_households, seed_s)
        mde_results = estimate_semilinear_mde(
            mus_sim, phi_bases, entropy_choo_siow
        )
        min_distance_results.append(mde_results)
        poisson_glm_results = choo_siow_poisson_glm(
            mus_sim, phi_bases, verbose=1
        )
...
def nested_logit_simul(
    ...
    additional_parameters = cast(
        tuple[list[list[int]], list[list[int]]],
        entropy_model.additional_parameters,
    )
    ...
    for s in range(n_simuls):
        seed_s = cast(int, child_seeds[s])
        mus_sim = nested_logit_instance.simulate(n_households, seed_s)
...
if __name__ == "__main__":
    ...
    if plot_choo_siow:
        ...
        outlier_simuls = choo_siow_results["Simulation"][
            outlier_mask
        ].unique()
        ...
        for true_val, ax in zip(true_betas, g.axes.ravel()):
            ax.vlines(
                true_val, *ax.get_ylim(), color="k", linestyles="dashed"
            )
        g.add_legend()
    ...
    if plot_nested_logit:
        ...
        plt.savefig(
            f"nested_logit_results_stderrs_N{n_households}_seed{seed}.png"
        )
